 ...
 * on a truss and a lit deck underneath. All of its CSS is styles/stage.css.
 *
 * Everything in the backdrop is decoration and is hidden from assistive
 * tech — the meaning lives in the content layered on top. It is also
 * entirely CSS: no canvas, no animation frames, no measurement, so it
 * server-renders as-is and costs nothing to hydrate.
 *
 * The cue (engaging one position dims the other two) is CSS `:has()` rather
 * than React state — no JS, and it works before hydrate.
 */
...
/** Matches the content grid in `<Stage>` exactly. */
const RIG_GRID = "rig-grid grid grid-cols-3 gap-6 px-6";

/**
 * Footlight bar heights. A fixed pattern, not random: the markup has to be
...
/**
 * Full bleed: these are properties of the venue, not of the rig, so they run
 * edge to edge however wide the display is.
 */
function Room() {
	return (
		<>
			...
			{/* Side booms — the wide, colourless cross-light that fills a room
			    ...
			    subject. Anchored radials, not linear gradients: a linear ramp
			    across a tall narrow box has not reached transparent by the time
			    it meets the box's inner edge, and drew a seam down the frame. */}
			<div
...
/**
 * Measured off --stage-w, exactly like the content underneath, so every beam
 * stays over its own performer at any width.
 */
function Rig() {
	return (
		<div className="stage-w absolute inset-y-0 left-1/2 -translate-x-1/2">
			...
			{/* Where the beams land. Same grid again, so a pool sits under its
			    own beam. Above the deck plane, so they read as light ON the
			    floor rather than pattern in it. */}
			<div
...
/**
 * Along the downstage edge, full bleed: they belong to the front of the
 * stage, not to any one instrument. Greyscale — the room responding, not a
 * fourth product.
 */
function Footlights() {
	return (
